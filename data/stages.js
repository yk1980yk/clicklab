...
/* ---------- 13 ---------- */
{
  ...
  choices: [
    ...
    { text: '個人のブログなので、企業の広告ではないと考える', score: 0,
      feedback: 'ブログの体裁は誰でも作れます。体裁は、広告かどうかの判断材料になりません。' }
  ],
...
/* ---------- 14 ---------- */
{
  ...
  choices: [
    ...
    { text: 'ゲーム内の通貨なので、実際のお金はかからない', score: 0,
      feedback: '有償通貨と明記されています。現金で購入したポイントを消費しています。' }
  ],
...
/* ---------- 16 ---------- */
{
  ...
  explanation:
    '完了していないのに完了と書く。表示の中でも特に悪質な部類です。ユーザーは安心して画面を閉じ、課金だけが続きます。日本では2022年6月施行の改正特定商取引法で、解約の妨害にあたる行為に罰則が設けられ、誤認させる表示によって申し込んだ場合には取消権も認められています。',
  checks: [
...
/* ---------- 18 ---------- */
{
  ...
  choices: [
    ...
    { text: '規約は長いので、申し込んでから読む', score: 0,
      feedback: '申し込んだ時点で同意したことになります。読む順番が逆です。' }
  ],
...
/* ---------- 19 （ホワイト） ---------- */
{
  ...
  explanation:
    '検体4とよく似た画面ですが、決定的に違うのは初期状態です。あちらはチェックが入っていて、外さない限り同意したことになりました。こちらは外れていて、合計にも含まれていない。関連商品を並べること自体は、探す手間を省く親切な設計です。',
  checks: [
